...
class Py3Mailer():
    ...
    def _retry_handler(self, recipient_data):
        ...
        csv_writer.writerow([
            ...
            recipient_data.get('column3'),
            recipient_data.get('column4')
    ...
    def _parse_csv(self, csv_path=None):
        ...
        for i, row in enumerate(csv_reader):
            ...
            try:
                ...
                recipient_column3 = row[2]
                recipient_column4 = row[3]
            ...
            if not recipient_email:
                ...
                recipient_data_list.append({
                    ...
                    'column3': recipient_column3,
                    'column4': recipient_column4,
        ...
        for recipient_data in recipient_list:
            ...
            for nb in range(0, self.nb_emails_per_recipient):
                ...
                try:
                    ...
                    smtp_server.sendmail(sender, recipient, message.encode("utf8"))
                    smtp_server.close()
...
def main(sys_args):
    ...
    try:
        ...
        print(
            "Not enough argumants supplied. Py3Mailer requests 1 option and 3 arguments: ./py3mailer -s html_path csv_path subject")
        sys.exit()
    ...
    py3mailer = Py3Mailer(html_path, csv_path, subject)

    if action == '-s':
        if input(
                "You are about to send to %s recipients. Do you want to continue (yes/no)? " % py3mailer.count_recipients()) in '[yes,y]':
            # save the csv file used to the stats file
            py3mailer._stats("CSV USED: %s" % csv_path)

            # send the email and try resend to failed recipients
            py3mailer.send()
            py3mailer.resend_failed()
        else:
            print("Aborted.")
            sys.exit()
        ...
        if input(
                "You are about to send a test mail to all recipients as specified in config.py. Do you want to continue (yes/no)? ") in '[yes,y]':
            py3mailer.send_test()
        else:
            print("Aborted.")
            sys.exit()
        ...
        print(
            "%s option is not supported. Use either [-s] to send to all recipients or [-t] to send to test recipients" % action)

    # save the end time to the stats file
    py3mailer._stats("END TIME: %s" % datetime.now())
